CREATE DATABASE IF NOT EXISTS Netflix_Ex;
USE Netflix_Ex;
...
from actores 
where sueldo > (select sueldo from actores where nombre like "Noa")
...
from actores
where day(fecha_nac) = month(fecha_nac) and id_actor in (select actor from peliculas where director_peli in ("Jon", "Adrian", "Nuria", "Ivan"))
order by ACT;
...
select *
from peliculas
where nombre_peli like "%o%o%" and duracion < (select duracion from peliculas where nombre_peli like "Animales nocturnos") and nota_peli > (select nota_peli from peliculas where actor = (select id_actor from actores where nombre like "Carlos"))
